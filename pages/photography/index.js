...
export default function PhotographyIndex() {
  ...
  useEffect(() => {
    fetch("../api/trackers/Photography_trackIps")
  ...
  useEffect(() => {
    DeclareStorageVariable("session", "Photography Visits", TOTAL_UNIQUE_IPS);
  }, [TOTAL_UNIQUE_IPS]);

  // Writing data to console
  useEffect(() => {
    if (TOTAL_UNIQUE_IPS !== null) {
      console.log("Total Visits on Photography Website: " + TOTAL_UNIQUE_IPS);
    }
